 ...
 * Lllll
 *
 * This is a validation helper class for Email Subscriptions (EmailSubsErrorHelper).
 * It checks the data submitted from the Email Subscription add/edit forms.
...
@RequestScoped
public class EmailSubsErrorHelper
{
    @Inject
    ...
    public boolean checkEmailSubsData(HttpServletRequest req)
    {
        ...
        if (checkData(email, "email", "Email cannot be empty", req) == true)
        {
            errors = true;
        }
        ...
        for ( int x = 0; x < opRows; x++) {
          ...
          if (ids.add(id) == false) {
             req.setAttribute("msg", "Duplicate EmailSubs on Row " + (x + 1));
             errors = true;
             break;
          }
        }
