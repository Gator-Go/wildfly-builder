...
public class EmailSubsDataHelper
{
   ...
   /**
    * This method loads Event EmailSubs selection data to the req attributes.
    *
    * @param req A HttpServletRequest object to be loaded with default data.
    */
   public void loadEventEmailSubsData(HttpServletRequest req)
   {
   ...
   public void loadEmailSubsDataFromReq(HttpServletRequest req, String newEmailSubsId)
   {
      ...
      List<EmailSubsFmt> emailSubsFmts = new ArrayList<EmailSubsFmt>();
      if (newEmailSubsId != null) {
        Long adminEventId = new Long(newEmailSubsId);
        AdminEvent adminEvent = eventService.getAdminEvent(adminEventId);

        EmailSubsFmt emailSubsFmt = new EmailSubsFmt();
        emailSubsFmt.setDeleteFlag("false");
        emailSubsFmt.setId(adminEvent.getId().toString());
        emailSubsFmt.setName(adminEvent.getName());
        emailSubsFmt.setDescription(adminEvent.getDescription());
        emailSubsFmts.add(emailSubsFmt);
      }
      ...
      for ( int x = 0; x < opRows; x++) {
        ...
        EmailSubsFmt emailSubsFmt = new EmailSubsFmt();
        emailSubsFmt.setDeleteFlag(deleteFlag);
        emailSubsFmt.setId(id);
        emailSubsFmt.setName(name);
        emailSubsFmt.setDescription(description);
        emailSubsFmts.add(emailSubsFmt);
      }
      req.setAttribute("emailSubsFmts", emailSubsFmts);
      String opSize = "" + emailSubsFmts.size();
   ...
   /**
    * This method loads data from a req object to a AdminEmailSubs object.
    *
    * @param adminEmailSubs A AdminEmailSubs object to be loaded with data.
    * @param req A HttpServletRequest object containing the data.
    */

   public void loadUserEmail(UserEmail userEmail, HttpServletRequest req)
   {

      if(!req.getParameter("firstName").isEmpty())
      {
         String firstName = req.getParameter("firstName");
         userEmail.setFirstName(firstName);
      }

      if(!req.getParameter("lastName").isEmpty())
      {
         String lastName = req.getParameter("lastName");
         userEmail.setLastName(lastName);
      }
   ...
   /**
    * This method loads data from a AdminEmailSubs object to a req object.
    *
    * @param adminEmailSubs A AdminEmailSubs object containing the data.
    * @param req A HttpServletRequest object to be loaded with data.
    */

   public void loadEmailSubsFormData(UserEmail userEmail, HttpServletRequest req)
   {
      req.setAttribute("id", userEmail.getId().toString());
      req.setAttribute("firstName", userEmail.getFirstName());
      req.setAttribute("lastName", userEmail.getLastName());
      req.setAttribute("email", userEmail.getEmail());

      List<EmailSubsFmt> emailSubsFmts = new ArrayList<EmailSubsFmt>();
      Set<AdminEvent> events = userEmail.getEvents();
